...
public class ConnessioniController {
	...
	@Autowired
	private ConnessioneEventPublisherService connessioneEventPublisherService;
	...
	public ConnessioneResponse createConnessione(@RequestBody CreateConnessioneRequest request) {
		...
		if (connessione != null) {
			...
			connessioneEventPublisherService.publish(connessione);
			return response;
		} else {
	...
	public Collection<ConnessioneResponse> getConnessioniByUtenteAndRuolo(@PathVariable String utente,
	...
	public ConnessioneResponse deleteConnessione(@RequestBody DeleteConnessioneRequest request) {
		...
		if (connessione != null) {
			ConnessioneResponse response = toConnessioneResponse(connessione);
			logger.info("REST CALL: deleteConnessione " + utente + ", " + seguito + ", " + ruolo + " --> " + response
					+ " *** DELETED ***");
			connessioneEventPublisherService.publish(connessione.getId());
			return response;
		} else {
	...
	/*
	 * Converte una collezione di connessioni in una collezione di risposte
	 * connessione.
	 */
	private Collection<ConnessioneResponse> toConnessioniResponse(Collection<Connessione> connessioni) {
		Collection<ConnessioneResponse> response = connessioni
				.stream()
